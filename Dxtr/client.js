...
function updateChart(init, force) {
	...
	if (force || (prevChartWidth != chartWidth) || (prevChartHeight != chartHeight)) {
		...
		if (init) {
			...
			focus.append('line')
				...
				.attr('y1', yScale(scaleBg(highThreshold)))
				.attr('x2', xScale(new Date(Date.now())))
				.attr('y2', yScale(scaleBg(highThreshold)))
			...
			focus.append('line')
				.attr('class', 'low-line')
				.attr('x1', xScale(new Date(Date.now() - FOCUS_DATA_RANGE_MS)))
				.attr('y1', yScale(scaleBg(lowThreshold)))
				.attr('x2', xScale(new Date(Date.now())))
				.attr('y2', yScale(scaleBg(lowThreshold)))
				.style('stroke-dasharray', ('6, 1'))
				.attr('stroke', '#c30909');
...
function updateLowThreshold(threshold) {
...
function updateHighThreshold(threshold) {
  if (highThreshold != threshold) {
...
function updateData(newData) {
	...
	if (newData != null) {
		data = $.parseJSON(newData).map(function(obj) {
			return {
				date: new Date(obj.date),
				sgv: scaleBg(obj.sgv),
				color: sgvToColor(obj.sgv),
...
function updateUnits(isMmol) {
	...
	if (isMmol && units != "mmol") {
		...
		data = data.map(function(obj) {
			return {
				date: new Date(obj.date),
				sgv: (Math.round((obj.sgv / 18) * 10) / 10).toFixed(1),
				color: obj.color,
				type: 'sgv'
			}
		});
